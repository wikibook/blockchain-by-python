...
def mk_pubkey_script(addr):
    ...
    # 0xa9 : OP_HASH160 : The input is hashed twice: first with SHA-256 and then with RIPEMD-160. 
    # 0x14 : N/A : The next opcode bytes is data to be pushed onto the stack 
    # 0x88 : OP_EQUALVERIFY : Same as OP_EQUAL, but runs OP_VERIFY afterward.
    # 0xac : OP_CHECKSIG : 
    # b58check_to_hex(addr) : 지갑 주소를 160-bit public key hash로 변환한다
...
def sign(tx, i, priv, hashcode=SIGHASH_ALL):
    ...
    # mk_pubkey_script(address) : OP_DUP OP_HASH160 <hash160> OP_EQUALVERIFY OP_CHECKSIG
    # sign하기 전에 scriptSig는 blank가 아니라 UTXO의 scriptPubKey 값을 복사해 넣는다.
    # 이건 technical reason이 있어서가 아니라 satoshi가 이렇게 했기 때문이다 (historical reason)
    # i-번째 이외의 script는 blank 이다
    script = mk_pubkey_script(address)
    
    # script를 input의 스크립트에 끼워 넣는다
