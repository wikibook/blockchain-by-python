...
def bci_unspent(*args):
    ...
    for a in addrs:
        try:
            data = make_request('https://testnet.blockchain.info/unspent?active='+a)
        except Exception as e:
    ...
    for addr in addrs:
        ...
        while 1:
            ...
            while not gathered:
                try:
                    data = make_request(
                        'https://testnet.blockchain.info/address/%s?format=json&offset=%s' %
                        (addr, offset))
                    gathered = True
                except Exception as e:
                    try:
...
# Pushes a transaction to the network using https://testnet.blockchain.info/pushtx
def bci_pushtx(tx):
...
def blockr_pushtx(tx, network='testnet'):
    if network == 'testnet':
...
def last_block_height(network='btc'):
    ...
    data = make_request('https://testnet.blockchain.info/latestblock')
    jsonobj = json.loads(data.decode("utf-8"))
...
def firstbits(address):
    if len(address) >= 25:
        return make_request('https://testnet.blockchain.info/q/getfirstbits/'+address)
    else:
        return make_request(
            'https://testnet.blockchain.info/q/resolvefirstbits/'+address)


def get_block_at_height(height):
    j = json.loads(make_request("https://testnet.blockchain.info/block-height/" +
...
def _get_block(inp):
    if len(str(inp)) < 64:
        ...
        return json.loads(make_request(
                          'https://testnet.blockchain.info/rawblock/'+inp).decode("utf-8"))
...
def get_block_height(txhash):
    j = json.loads(make_request('https://testnet.blockchain.info/rawtx/'+txhash).decode("utf-8"))
